...
//  代码来自 https://github.com/SunnyApp/flutter_contact/blob/master/ios/Classes/SwiftFlutterContactPlugin.swift
//
//  Created by wind.wang on 2020/10/9.
//
...
//        result["displayName"] = CNContactFormatter.string(from: contact, style: CNContactFormatterStyle.fullName)
		...
		if contact.isKeyAvailable(CNContactNamePrefixKey){
			result["prefix"] = contact.middleName
		}
		if contact.isKeyAvailable(CNContactNameSuffixKey){
			result["suffix"] = contact.middleName
		}
		...
		if contact.isKeyAvailable(CNContactJobTitleKey){
			result["jobTitle"] = contact.middleName
		}
...
extension CNMutableContact {
    func takeFromDictionary(_ dictionary: [String:Any?]) {
        ...
        //Simple fields
//        contact.identifier = dictionary["identifier"] as? String ?? ""
        contact.givenName = dictionary["firstName"] as? String ?? ""
        contact.familyName = dictionary["lastName"] as? String ?? ""
        ...
        //Phone numbers
        if let phoneNumbers = dictionary["phoneNumbers"] as? [[String:String]] {
